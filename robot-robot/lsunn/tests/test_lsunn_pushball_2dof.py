"""
Test LS-UNN PPO policy trained on PushBall 2-DoF.
"""
...
N_JOINTS = 2  # nombre d'articulations du bras (indépendant du state_dim du VAE)


# -------------------------------------------------
# Charger le VAE
# -------------------------------------------------
# NB: on n'impose plus state_dim/latent_dim en dur (DEFAULT_LATENT_DIM / arm_obs=6).
...
ckpt_path = DATA_DIR / "base_2dof.pt"
...
env = VecNormalize.load(
    str(DATA_DIR / "lsunn_2dof" / "vec_normalize.pkl"),
    env,
)
...
model = PPO.load(
    DATA_DIR / "lsunn_2dof" / "policy",
    env=env,
    device=DEVICE,
)

print("LS-UNN PPO 2DoF loaded.")
