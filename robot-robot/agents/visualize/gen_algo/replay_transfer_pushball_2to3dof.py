...
# ── Arguments ─────────────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="Replay transfer pushball 2→3 DOF")
...
data_dir = f"runs/transfer_pushball_2to3dof/{mode}"

# ── Chargement ────────────────────────────────────────────────────────────────
files = sorted(glob.glob(os.path.join(data_dir, "ep_*.npz")))
if not files:
    raise FileNotFoundError(
        f"Aucun fichier dans '{data_dir}'. Lancez runs_transfer_pushball_2to3dof.py d'abord."
    )
...
print(f"Replay [{mode.upper()}] — {len(files)} épisode(s) — transfer pushball 2→3 DOF\n")
