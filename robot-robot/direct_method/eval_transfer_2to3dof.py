"""Évaluation quantitative du transfert push-ball : PPO 2DoF exécutée sur le bras 3DoF.

Chaîne strictement identique à visualize_transfer_2to3dof.py (même env brut,
...
l'écart entre la vitesse d'effecteur commandée par la politique (J2 sur le bras
2DoF virtuel) et celle réellement exécutée par le 3DoF (J3). Si le mapping
d'action est bon, l'erreur relative tend vers 0 et le cosinus vers 1.

Usage :
    cd robot-robot && python3 -m direct_method.eval_transfer_2to3dof \
        --mapper conditioned --episodes 200 --seed 0
"""
...
from direct_method.transfer_pushball_2to3dof import (
...
def main():
    p = argparse.ArgumentParser(
        description="Taux de réussite du transfert push-ball 2DoF -> 3DoF.")
    ...
    print(f"Transfert push-ball 3DoF <- PPO 2DoF | action={args.mapper} | "
    ...
    print()
    print("=" * 52)
    print(f"  Sens                  : 3DoF réel <- politique 2DoF")
