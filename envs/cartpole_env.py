...
class CartPoleVectorEnv(VectorEnv):
    ...
    def reset(
        ...
        *,
        ...
        # Bornes par défaut pour x, x_dot, theta_dot
        self.low, self.high = utils.maybe_parse_reset_bounds(options, -0.05, 0.05)

        # État initial aléatoire standard
        self.state = self.np_random.uniform(
            low=self.low, high=self.high, size=(4, self.num_envs)
        )

        # Angle initial spécifique ±30°
        angle_max = np.pi / 6   # 30°
        self.state[2, :] = self.np_random.uniform(-angle_max, angle_max, size=(self.num_envs,))
