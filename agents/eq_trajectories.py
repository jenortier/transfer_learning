...
================================================
Part 1: Generate aligned trajectories (2-DoF ↔ 3-DoF) using TRAINED PPO POLICIES.

Key improvement over v1:
  - Uses actual PPO policies instead of a proportional controller.
    The proportional controller outputs Cartesian directions; the PPO policy outputs
    joint-space deltas. Using the real policies means the action mapper learns the
    correct correspondence that will be used at transfer time.
  - Longer trajectories (20 steps/target) for richer coverage.
  - More pairs (200) for better mapper generalisation.
  - Same target sequence for both arms → true temporal alignment.

Alignment protocol:
  1. Both arms reset with the same seed (→ same θ1, θ2 initial angles).
  2. A common target sequence is generated from the 2-DoF initial effector position.
  3. The same targets are injected into both envs at each target-change event.
  4. Each arm uses its own trained PPO policy to act.
  5. (raw_obs_2dof_t, raw_obs_3dof_t), (action_2dof_t, action_3dof_t) are recorded.
"""
...
def generate_one_pair(
    ...
    # --- Create raw envs ---
    env_2dof = Arm2DoFEnv(render_mode=None)
    env_3dof = Arm3DoFEnv(render_mode=None)
...
def main():
    ...
    N_TARGETS        = 100
    STEPS_PER_TARGET = 20     # longer windows → more diverse state coverage
    ...
    # ----- Generate pairs -----
    total_steps_per_pair = N_TARGETS * STEPS_PER_TARGET
